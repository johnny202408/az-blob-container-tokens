...
# -------------------------
locals {
  write_sas_file      = "${path.module}/write_sas_token.txt"
  readdelete_sas_file = "${path.module}/read_delete_sas_token.txt"
}

# -------------------------
# Local files for SAS tokens
# -------------------------
resource "local_file" "sas_write_file" {
  filename = local.write_sas_file
  content  = data.azurerm_storage_account_blob_container_sas.write_sas.sas
}

resource "local_file" "sas_read_delete_file" {
  filename = local.readdelete_sas_file
  content  = data.azurerm_storage_account_blob_container_sas.read_delete_sas.sas
}

# -------------------------
# Outputs
# -------------------------
output "write_sas_file_path" {
  description = "Path to the file containing the write SAS token"
  value       = local_file.sas_write_file.filename
}

output "read_delete_sas_file_path" {
  description = "Path to the file containing the read/delete SAS token"
  value       = local_file.sas_read_delete_file.filename
}
